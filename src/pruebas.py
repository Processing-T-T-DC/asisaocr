...
def procesar_un_html(html_content, nombre_archivo):
    """
    ...
    vars_cols = {i: nombre for i, nombre in enumerate(COLUMNAS_ORDENADAS)}
    ...
    resultado = {col: [] for col in COLUMNAS_ORDENADAS}
...
def procesar_carpeta_htmls(ruta_carpeta, nombre_csv_salida="consolidado_pias.csv"):
...
def ejecutar_inspeccion(ruta_carpeta):
    archivos = glob.glob(os.path.join(ruta_carpeta, "*.html"))
    todos_los_hallazgos = []

    for archivo in archivos:
        with open(archivo, 'r', encoding='utf-8') as f:
            resultado = inspeccion_cruda_html(f.read(), os.path.basename(archivo))
...
procesar_carpeta_htmls(ruta_mis_htmls)
